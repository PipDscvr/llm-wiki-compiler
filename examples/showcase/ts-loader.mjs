/**
 * @file examples/showcase/ts-loader.mjs
 ...
 *
 * WHY this exists: the seed script must reuse the project's real writers
 * (`writeState`, `writeCandidate`) rather than hand-rolling their JSON, so the
 * example cannot silently rot when one of those formats changes. Those writers
 * are not part of the published SDK surface (`dist/index.js`), and the bundled
 * `dist/cli.js` exports nothing, so importing `src/**.ts` is the only way to
 * reach them.
 *
