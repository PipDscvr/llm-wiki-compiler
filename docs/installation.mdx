...
<Tabs>
  ...
  <Tab title="Ollama">
    ...
    **Different backends for chat and embeddings**
    ...
    Embeddings can come from a different provider than chat — for example Claude Agent SDK for generation with a local vLLM instance serving vectors:
    ...
    A self-hosted endpoint needs no API key. Without `OPENAI_EMBEDDINGS_BASE_URL`, `OPENAI_API_KEY` is required.
