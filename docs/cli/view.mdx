...
| `#/<entity-type>` | Pages of one entity type your profile declares, such as `#/articles` |
...
`#/<entity-type>` accepts only the entity types your active profile actually declares - anything else falls back to the Overview dashboard. On a project running the built-in default profile, no entity type is declared and no such route exists.
...
Each entity type your profile declares gets a row in the sidebar's **BROWSE** section and a list route of its own at `#/<entity-type>`, and every page on it opens at `#/<entity-type>/<slug>`. See **Sidebar navigation** below for how BROWSE is built.
