...
| `LLMWIKI_EMBEDDING_MODEL` | Provider default | Embedding model to use. Applies when the embedding provider is `openai` or `ollama`, or whenever `LLMWIKI_EMBEDDING_PROVIDER` is set |
